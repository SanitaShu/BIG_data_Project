...
def load_data():
    ...
    # Fallback synthetic data with full month names
    if df is None:
...
# lead_time bounds from data (if present)
...
# Cancellation rate by month (full month names)
if "is_canceled" in df_f.columns and "arrival_date_month" in df_f.columns and len(df_f):
    ...
    tmp["arrival_date_month"] = pd.Categorical(
        tmp["arrival_date_month"],
        categories=month_order,
        ordered=True,
    )
    tmp = tmp.sort_values("arrival_date_month").dropna()
