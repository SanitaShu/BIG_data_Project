...
st.set_page_config(page_title="Project 7 — UI Prototype", layout="wide")
st.title("Project 7: User/Data Interaction — UI Prototype")
st.caption("Cleaned ADR (>0), integer sliders for people counts, reset button, and small UX fixes.")

# Data Loading
@st.cache_data
def load_data():
    # Try to load a real file; otherwise synthesize a small demo set
    candidates = [
    ...
    if df is None:
        ...
        df = pd.DataFrame({
            ...
            "arrival_date_month": rng.choice(["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"], size=n),
            "lead_time": rng.integers(0, 365, size=n),
            "adults": rng.integers(1, 4, size=n),
            "children": rng.integers(0, 3, size=n),
            "weekend_nights": rng.integers(0, 3, size=n),
            "week_nights": rng.integers(0, 5, size=n),
            ...
            "is_canceled": rng.choice([0,1], size=n, p=[0.7,0.3])
        })
    # Cleaning rules
...
# Categorical filters (limit to keep UI tidy)
...
for c in sorted(num_cols)[:8]:
    ...
    is_count = c.lower() in {"adults","children","week_nights","weekend_nights","lead_time","stays_in_weekend_nights","stays_in_week_nights"}
    if is_count:
        lo, hi = st.sidebar.slider(f"{c} range", min_value=int(vmin), max_value=int(vmax),
                                   value=(int(vmin), int(vmax)), step=1, key=f"num_{c}")
    else:
        lo, hi = st.sidebar.slider(f"{c} range", min_value=vmin, max_value=vmax,
                                   value=(vmin, vmax), key=f"num_{c}")
    range_filters[c] = (lo, hi)

# Keep ADR > 0 (already cleaned; toggle kept for clarity)
only_pos_adr = st.sidebar.checkbox("Only positive ADR", value=True)
...
for c, (lo, hi) in range_filters.items():
    if c in {"adults","children","week_nights","weekend_nights","lead_time","stays_in_weekend_nights","stays_in_week_nights"}:
...
if query and cat_cols:
    contains = pd.Series([False]*len(df_f))
    for c in cat_cols:
...
if "adr" in df_f.columns and len(df_f):
    c4.metric("Avg ADR", f"{df_f['adr'].mean():.2f}")
else:
    c4.metric("Avg ADR", "—")
...
if "is_canceled" in df_f.columns and "arrival_date_month" in df_f.columns and len(df_f):
    ...
    if len(tmp):
        chart1 = alt.Chart(tmp).mark_bar().encode(
            ...
            y=alt.Y("is_canceled:Q", title="Cancellation Rate", axis=alt.Axis(format='%')),
...
if len(df_f):
    ...
    st.download_button("Download filtered CSV", csv, file_name="filtered_output.csv", mime="text/csv")
else:
