...
def load_data():
    """
    Load hotel bookings data. If a real CSV is found, use it.
...
for c in sorted(num_cols):
    ...
    if c in {"adults", "children"}:
        # fixed ranges: adults 1–4, children 0–7
        if c == "adults":
            vmin, vmax = 1, 4
        if c == "children":
            vmin, vmax = 0, 7
