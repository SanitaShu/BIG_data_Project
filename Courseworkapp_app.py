...
# ---- NUMERIC FILTERS (EXCLUDING SOME) ----
range_filters = {}

# we will SKIP these from numeric sliders
skip_numeric = {"stays_in_week_nights", "stays_in_weekend_nights", "is_canceled"}

# max limits and pretty labels for specific columns
pretty_label = {
...
max_override = {
    ...
    "adults": 4,     # up to 4
    "children": 7,   # up to 7
    # lead_time left as real max (can be long)
}

count_like = {"adults", "children"}
...
for c in sorted(num_cols):
    ...
    if c in max_override:
        vmax = min(vmax, max_override[c])

    label = pretty_label.get(c, f"{c} range")
    is_count = c in count_like
...
# ---- SPECIAL FILTER FOR CANCELLATION (Yes/No) ----
cancel_option = None
...
if charts:
    ...
    for col, (title, ch) in zip(cols, charts):
        with col:
            # bigger, bold titles for each chart
            st.markdown(f"### **{title}**")
