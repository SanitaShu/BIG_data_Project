...
# For lead_time, use actual data bounds for validation
...
for c in sorted(num_cols):
    ...
    # ---------- SPECIAL: LEAD_TIME AS MANUAL INPUTS ----------
    if c == "lead_time":
    ...
    if c in {"adults", "children"}:
        # fixed ranges you wanted: adults 1–4, children 0–7
        if c == "adults":
            vmin, vmax = 1, 4
        if c == "children":
            vmin, vmax = 0, 7
...
if charts:
    ...
    for col, (title, ch) in zip(cols, charts):
        with col:
            st.markdown(f"### **{title}**")  # bigger, bold
