...
def load_data():
    """
    Load hotel bookings data. If no real CSV is found, generate
    a synthetic demo dataset with the same schema.
    """
    candidates = [
    ...
    # Fallback synthetic data with a schema similar to the real dataset
    if df is None:
        ...
        df = pd.DataFrame({
            ...
            "is_canceled": rng.integers(0, 2, size=n)  # 0 = not cancelled, 1 = cancelled
        })
...
# columns we DO NOT show sliders for
...
# enforced visual ranges for some sliders
manual_ranges = {
    "adr": (0, 500),     # always 0–500
    "adults": (1, 4),    # always 1–4
    "children": (0, 7)   # always 0–7
}
...
for c in sorted(num_cols):
    ...
    # override with our manual ranges if specified
    if c in manual_ranges:
        vmin, vmax = manual_ranges[c]

    label = pretty_label.get(c, f"{c} range")

    # adults & children are integer counts; others can be float
